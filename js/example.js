...
function is_element_in_list( item, list ){
  console.log(list);
  for ( var i = 0 ; i < list.childElementCount ; i++){
    console.log( i );
    if ( item == list.children[i].id ){
...
dragula([$('left-events'), $('right-events')])
  ...
  .on('drop', function (el, container) {
    ...
    // remove any element with class="empty"
    // Find if there is any empty class element
    if ( container.id == "right-events") {


      for ( var i = 0  ; i < container.childElementCount ; i++){
        if ( container.children[i].className == "empty" ){
      ...
      if ( jQuery(el).hasClass("nw_chapter") ) {

        var chapter = "chapter_content"+el.id;
        var chapter_cont = document.getElementById(chapter);
        for ( var ii = 0 ; ii < chapter_cont.childElementCount ; i++ ){
          if ( !is_element_in_list( document.getElementById("right-events").children[ii].id, chapter_cont  ) )

// item
// document.getElementById("right-events").children[ii].className == "nw_post"  &&
// document.getElementById("right-events").children[ii].id == 

// list
// chapter_cont.children[ii].id
// chapter_cont.children[ii].className == "nw_post"

            document.getElementById("right-events").appendChild(chapter_cont.children[ii])

        }
        document.getElementById("right-events").getElementsByClassName("nw_chapter")[0].remove()
      }
    }

    if ( container.id == "left-events" && el.className.indexOf("empty") == 0 ) {
      for ( var i = 0  ; i < container.childElementCount ; i++)
        if ( container.children[i].className.indexOf("empty") == 0 ){
          container.children[i].remove()
          break;
        }
    }
